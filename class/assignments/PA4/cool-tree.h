...
extern Symbol No_type, Bool, Int, Str, Object, self, No_class, SELF_TYPE, No_type;
...
public:
   ...
   virtual Symbol get_name() { return 0; }
   virtual Symbol get_parent() { return 0; }
   virtual int accumulate_types(SymbolTable<Symbol, Class__class>* classmap) = 0;
   virtual int typecheck(SymbolTable<Symbol, Class__class>* classmap) = 0;
   virtual std::map<Symbol, std::pair<Formals, Symbol>> get_methods() = 0;
   virtual std::map<Symbol, Symbol> get_attrs() = 0;
   virtual std::pair<Formals, Symbol> get_method_type(Symbol name) = 0;
   virtual Symbol get_attr_type(Symbol name) = 0;
   virtual bool is_descendant_of(Symbol needle) = 0;
...
public:
   ...
   virtual Symbol get_name() { return 0; }
   virtual Symbol get_type() { return 0; }
   virtual Formals get_formals() { return 0; }
   virtual bool is_method() { return false; }
   virtual Expression get_expr() { return 0; }
...
public:
   ...
   virtual int typecheck(SymbolTable<Symbol, Class__class>* classmap, SymbolTable<Symbol, Symbol> *scope) = 0;
...
protected:
   ...
   std::map<Symbol, std::pair<Formals, Symbol>> methods;
public:
   ...
   virtual int accumulate_types(SymbolTable<Symbol, Class__class>* classmap) {
       if (parent == Str || parent == Int || parent == Bool || parent == SELF_TYPE) return 1;
       if (parent != No_class) {
           parent_class = classmap->lookup(parent);
           methods = parent_class->get_methods();
           attrs = parent_class->get_attrs();
       }
       for (int i = 0; i < features->len(); ++i) {
           auto feature = features->nth(i);
           if (feature->is_method()) {
               auto formals = feature->get_formals();
               auto prev_formals = feature->get_formals();
               if (methods.find(feature->get_name()) != methods.end()) {
                   prev_formals = methods[feature->get_name()].first;
               }
               if (prev_formals->len() != formals->len()) return 1;
               std::set<Symbol> formalset;
               for (int j = 0; j < formals->len(); ++j) {
                   formalset.insert(formals->nth(j)->get_name());
                   if (formals->nth(j)->get_name() == self) return 1;
                   if (formals->nth(j)->get_type() == SELF_TYPE) return 1;
                   if (formals->nth(j)->get_type() != prev_formals->nth(j)->get_type()) return 1;
               }
               if (formalset.size() != formals->len()) return 1;
               methods[feature->get_name()] = std::make_pair(feature->get_formals(), feature->get_type());
           } else {
               if (attrs.find(feature->get_name()) != attrs.end()) return 1;
               if (feature->get_name() == self) return 1;
               attrs[feature->get_name()] = feature->get_type();
           }
       }
       return 0;
   }

   virtual int typecheck(SymbolTable<Symbol, Class__class>* classmap) {
       SymbolTable<Symbol, Symbol> *scope = new SymbolTable<Symbol, Symbol>;
       scope->enterscope();
       scope->addid(self, &SELF_TYPE);
       classmap->enterscope();
       classmap->addid(SELF_TYPE, this);
       for (int i = 0; i < features->len(); ++i) {
           auto feature = features->nth(i);
           if (!feature->is_method()) {
               int errors = feature->get_expr()->typecheck(classmap, scope);
               if (errors) return errors;
           } else {
               scope->enterscope();
               auto formals = feature->get_formals();
               if (!classmap->lookup(feature->get_type())) return 1;
               std::vector<Symbol> types;
               for (int j = 0; j < formals->len(); ++j) {
                   types.push_back(formals->nth(j)->get_type());
               }
               for (int j = 0; j < formals->len(); ++j) {
                   scope->addid(formals->nth(j)->get_name(), &types[j]);
               }
               int errors = feature->get_expr()->typecheck(classmap, scope);
               if (errors) return errors;
               auto ty = feature->get_expr()->type;
               auto cl = classmap->lookup(ty);
               if (ty != No_type && ty != feature->get_type() && (!cl || !cl->is_descendant_of(feature->get_type()))) return 1;
               scope->exitscope();
           }
       }
       scope->exitscope();
   ...
   virtual std::map<Symbol, std::pair<Formals, Symbol>> get_methods() { return methods; }
   virtual std::map<Symbol, Symbol> get_attrs() { return attrs; }
   virtual std::pair<Formals, Symbol> get_method_type(Symbol name) { return methods[name]; }
   virtual Symbol get_attr_type(Symbol name) { return attrs[name]; }
   virtual bool is_descendant_of(Symbol needle) {
       if (name == needle || parent == needle) return true;
       if (parent == No_class) return false;
       return parent_class->is_descendant_of(needle);
   }
...
public:
   ...
   void dump(ostream& stream, int n);
   virtual int typecheck(SymbolTable<Symbol, Class__class>* classmap, SymbolTable<Symbol, Symbol> *scope) {
       if (name == self) return 1;
       int errors = expr->typecheck(classmap, scope);
       auto type = scope->lookup(name);
       Symbol ty;
       if (!type) {
           auto myclass = classmap->lookup(*scope->lookup(self));
           if (!myclass) return 1;
           ty = myclass->get_attr_type(name);
       } else {
           ty = *type;
       }
       if (ty != expr->type) errors += 1;
       this->type = expr->type;
       if (this->type == SELF_TYPE) {
           this->type = *scope->lookup(self);
       }
       return errors;
   }
...
public:
   ...
   virtual int typecheck(SymbolTable<Symbol, Class__class>* classmap, SymbolTable<Symbol, Symbol> *scope) {
       int errors = expr->typecheck(classmap, scope);
       if (errors) return errors;
       auto type = classmap->lookup(type_name);
       auto cl = classmap->lookup(expr->type);
       if (!type || !cl || !cl->is_descendant_of(type_name)) return 1;
       auto method_type = type->get_method_type(name);
       if (!method_type.second) return 1;
       if (actual->len() != method_type.first->len()) return 1;
       for (int i = 0; i < actual->len(); ++i) {
           int errors = actual->nth(i)->typecheck(classmap, scope);
           if (errors) return errors;
           if (!classmap->lookup(actual->nth(i)->type)->is_descendant_of(method_type.first->nth(i)->get_type())) return 1;
       }
       this->type = method_type.second;
       if (this->type == SELF_TYPE) {
           this->type = type_name;
       }
       return 0;
...
public:
   ...
   virtual int typecheck(SymbolTable<Symbol, Class__class>* classmap, SymbolTable<Symbol, Symbol> *scope) {
       ...
       auto cl = classmap->lookup(expr->type);
       if (!cl) return 1;
       auto method_type = cl->get_method_type(name);
       if (!method_type.second) return 1;
       if (actual->len() != method_type.first->len()) return 1;
       for (int i = 0; i < actual->len(); ++i) {
           int errors = actual->nth(i)->typecheck(classmap, scope);
...
public:
   ...
   void dump(ostream& stream, int n);
   virtual int typecheck(SymbolTable<Symbol, Class__class>* classmap, SymbolTable<Symbol, Symbol> *scope) {
       int errors = pred->typecheck(classmap, scope) +
                    then_exp->typecheck(classmap, scope) +
                    else_exp->typecheck(classmap, scope);
       if (errors) return errors;

       if (pred->type != Bool) return 1;
       this->type = then_exp->type;
       auto cl = classmap->lookup(this->type);
       auto cl2 = classmap->lookup(else_exp->type);
       while (!cl2->is_descendant_of(this->type)) {
           this->type = cl->get_parent();
           cl = classmap->lookup(this->type);
       }
       return 0;
   }
...
public:
   ...
   void dump(ostream& stream, int n);
   virtual int typecheck(SymbolTable<Symbol, Class__class>* classmap, SymbolTable<Symbol, Symbol> *scope) {
       int errors = pred->typecheck(classmap, scope) + body->typecheck(classmap, scope);
       if (errors) return errors;
       if (pred->type != Bool) return 1;
       this->type = Object;
       return 0;
...
public:
   ...
   virtual int typecheck(SymbolTable<Symbol, Class__class>* classmap, SymbolTable<Symbol, Symbol> *scope) {
       int errors = expr->typecheck(classmap, scope);
       if (errors) return errors;
       std::set<Symbol> typeset;
       std::vector<Symbol> types;
       for (int i = 0; i < cases->len(); ++i) {
           typeset.insert(cases->nth(i)->get_type());
           types.push_back(cases->nth(i)->get_type());
       }
       if (types.size() != typeset.size()) return 1;
       this->type = No_type;
       for (int i = 0; i < cases->len(); ++i) {
           scope->enterscope();
           scope->addid(cases->nth(i)->get_name(), &types[i]);
           int errors = cases->nth(i)->get_expr()->typecheck(classmap, scope);
           if (errors) return errors;
           if (this->type == No_type) {
               this->type = cases->nth(i)->get_expr()->type;
           } else {
               auto cl = classmap->lookup(this->type);
               auto cl2 = classmap->lookup(cases->nth(i)->get_expr()->type);
               while (!cl2->is_descendant_of(this->type)) {
                   this->type = cl->get_parent();
                   cl = classmap->lookup(this->type);
               }
...
public:
   ...
   virtual int typecheck(SymbolTable<Symbol, Class__class>* classmap, SymbolTable<Symbol, Symbol> *scope) {
       ...
       for (int i = 0; i < body->len(); ++i) {
           errors += body->nth(i)->typecheck(classmap, scope);
           this->type = body->nth(i)->type;
       }
       scope->exitscope();
       return errors;
   }
...
public:
   ...
   void dump(ostream& stream, int n);
   virtual int typecheck(SymbolTable<Symbol, Class__class>* classmap, SymbolTable<Symbol, Symbol> *scope) {
       if (identifier == self) return 1;
       int errors = init->typecheck(classmap, scope);
       if (errors) return errors;
       if (init->type != No_type && init->type != type_decl) return 1;
       scope->enterscope();
       scope->addid(identifier, &type_decl);
       errors = body->typecheck(classmap, scope);
       scope->exitscope();
       this->type = body->type;
       return errors;
   }
...
public:
   ...
   void dump(ostream& stream, int n);
   virtual int typecheck(SymbolTable<Symbol, Class__class>* classmap, SymbolTable<Symbol, Symbol> *scope) {
       int errors = e1->typecheck(classmap, scope) + e2->typecheck(classmap, scope);
       if (errors) return errors;
       if (e1->type != Int || e2->type != Int) return 1;
       this->type = Int;
       return 0;
   }
...
public:
   ...
   void dump(ostream& stream, int n);
   virtual int typecheck(SymbolTable<Symbol, Class__class>* classmap, SymbolTable<Symbol, Symbol> *scope) {
       int errors = e1->typecheck(classmap, scope);
       if (errors) return errors;
       if (e1->type != Int) return 1;
       this->type = Int;
       return 0;
   }
...
public:
   ...
   virtual int typecheck(SymbolTable<Symbol, Class__class>* classmap, SymbolTable<Symbol, Symbol> *scope) {
       int errors = e1->typecheck(classmap, scope) + e2->typecheck(classmap, scope);
       std::set<Symbol> defaults = {Str, Int, Bool};
       if (errors) return errors;
       if (e1->type != e2->type && (defaults.find(e1->type) != defaults.end() || defaults.find(e2->type) != defaults.end()))
           return 1;
       this->type = Bool;
       return 0;
   }
...
public:
   ...
   virtual int typecheck(SymbolTable<Symbol, Class__class>* classmap, SymbolTable<Symbol, Symbol> *scope) {
       auto type = scope->lookup(name);
       if (!type) {
           auto self_type = scope->lookup(self);
           if (self_type && classmap->lookup(*self_type)) {
               this->type = classmap->lookup(*self_type)->get_attr_type(name);
